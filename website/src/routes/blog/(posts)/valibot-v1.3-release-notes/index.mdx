...
Huge thanks to [@EskiMojo14](https://github.com/EskiMojo14), [@alexilyaev](https://github.com/alexilyaev), and [@yslpn](https://github.com/yslpn) for their contributions to this release.

## Smarter pipelines

Two new additions make Valibot pipelines more expressive. <Link href="/api/guard/">`guard`</Link> lets you narrow values using an existing type predicate, and <Link href="/api/parseBoolean/">`parseBoolean`</Link> turns common boolish input into a real boolean.
...
One important detail from the implementation: primitive inputs are cached by value, while objects and functions are cached by reference identity. That means mutating an input object and reusing the same reference can return a stale cached result. For best results, use caching with immutable inputs and avoid mutating cached output objects.
...
For authentication flows, <Link href="/api/jwsCompact/">`jwsCompact`</Link> checks whether a string matches the three-part JWS compact serialization shape with unpadded Base64URL-like segments. This is a lightweight structural check, not full JWT verification.
